...
t("ملفات الشيفرة موجودة وتُحلّل", () => { for (const rel of srcs) { const p = path.join(DIR, rel); if (!fs.existsSync(p)) throw new Error(`مفقود ${rel}`); new Function(fs.readFileSync(p, "utf8")); } return srcs.join(" · "); });
...
const all = inline.join("\n") + srcs.map(x => fs.readFileSync(path.join(DIR, x), "utf8")).join("\n"), idSet = new Set(ids);
